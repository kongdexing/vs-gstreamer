...
static void
start_feed(GstElement* source, guint size, CustomData8* data)
{
    if (data->sourceid == 0) {
...
static void
stop_feed(GstElement* source, CustomData8* data)
{
    if (data->sourceid != 0) {
...
new_sample(GstElement* sink, CustomData8* data)
{
    GstSample* sample;
...
static void
error_cb(GstBus* bus, GstMessage* msg, CustomData8* data)
{
    GError* err;
    gchar* debug_info;
